...
/* Los músicos siempre se pueden eliminar, si no tienen composiciones o no figuran como
...
BEGIN
    ...
    RAISE_APPLICATION_ERROR(-20004,'Los músicos siempre se pueden eliminar, si no tienen composiciones o no figuran como organizadores de conciertos');
    END IF;
END El_MUSICIAN;
...
BEGIN
    ...
    IF tipo=3 THEN RAISE_APPLICATION_ERROR(-20007,'Los compositores sólo pueden componer un tipo de música y los interpretes pueden tocar máximo tres instrumentos');
END IF;
...
BEGIN
    ...
    :NEW.comp_type := type ;
    ELSE raise_application_error(-20050,'Si no se indica el tipo de música de un compositor, se supone que el tipo de música que
    sabe interpretar (si solamente es uno)');
END IF;
END;
